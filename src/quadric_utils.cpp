...
Quadric_Utils::Quadric_Utils() : cloud_(new pcl::PointCloud<pcl::PointXYZ>), num_neighbours_(20), kappa_max_(0), kappa_min_(0), a_(0), b_(0), c_(0), d_(0), e_(0), f_(0), index_central_point_(0), shell_variable_(0.2), added_shells_(4), index_offset_(0), index_stepsize_(50)
{
}
...
void Quadric_Utils::analyse_surface()
{
...
void Quadric_Utils::kappa_max_min()
{
	//Obtain curvature of hessian matrix from coefficients of the model function
	kappa_max_ =
	kappa_max_ = 0.5*(-a_-c_+ sqrt(pow(a_, 2) + 4*pow(b_, 2) - 2* a_*c_ + pow(c_, 2)));
	kappa_min_ = 0.5*(-a_-c_- sqrt(pow(a_, 2) + 4*pow(b_, 2) - 2* a_*c_ + pow(c_, 2)));
}

void Quadric_Utils::tangent_axes()
{
...
void Quadric_Utils::determine_modelfunction_coefficients(size_t point_index)
{
		/*
		#########################################
...
void Quadric_Utils::set_shell_descriptor_parameters()
{
...
std::vector<size_t> Quadric_Utils::develop_next_shell(int first_shell)
{
...
void Quadric_Utils::develop_feature(double point_index)
{
	...
	// std::cout << "Computation of first shell ready" << std::endl;
